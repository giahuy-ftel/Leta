...
void oled_hw_write_byte(uint8_t data, bool is_data)
{
	if (is_data) {
...
{
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_AFIO, ENABLE);
	RCC_APB2PeriphClockCmd(I2C_SCL_CLOCK | I2C_SDA_CLOCK, ENABLE);
	RCC_APB1PeriphClockCmd(RCC_APB1Periph_I2C1, ENABLE);
	GPIO_PinRemapConfig(GPIO_Remap_I2C1, DISABLE);

	GPIO_InitTypeDef GPIO_InitStruct;
	GPIO_InitStruct.GPIO_Pin = I2C_SCL_PIN | I2C_SDA_PIN;		// For I2C
...
static uint8_t i2c_hw_write_data(uint8_t data)
{
	uint16_t I2C_Timeout = TIME_OUT;
	while (!I2C_GetFlagStatus(I2C1, I2C_FLAG_TXE) && I2C_Timeout) {
		I2C_Timeout--;
	}	
	if(I2C_Timeout == 0) return 0;
...
void i2c_hw_write(uint8_t address, uint8_t reg, uint8_t data) {
	if(!i2c_hw_start(address, I2C_Direction_Transmitter, DISABLE)) return;
	if(!i2c_hw_write_data(reg)) return;
	if(!i2c_hw_write_data(data)) return;
	i2c_hw_stop();
}

uint8_t i2c_hw_read(uint8_t address, uint8_t reg) {
	uint8_t received_data;
	if(!i2c_hw_start(address, I2C_Direction_Transmitter, ENABLE)) return 0;
	if(!i2c_hw_write_data(reg)) return 0;	//EV8_1
	//i2c_hw_stop();
	if(!i2c_hw_start(address, I2C_Direction_Receiver, DISABLE)) return 0;
...
void i2c_hw_read_multi(uint8_t address, uint8_t reg, uint8_t* data, uint16_t count) {
	...
	if(!i2c_hw_start(address, I2C_Direction_Receiver, ENABLE)) return;
	while (count--) {
		if (!count) {
			/* Last byte */
