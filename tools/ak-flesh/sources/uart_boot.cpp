...
typedef enum {
	FLASH_TARGET_INTERNAL = 0,
	FLASH_TARGET_EXTERNAL,
} flash_target_mode_t;
...
/* help string */
static string help_string("Help: ak-flesh /dev/ttyUSB0 <firmware.bin|app-folder> <address> [internal|external]");

/* firmware file */
static string firmware_path;
static string icon_path_override;
static firmware_header_t file_firmware_header;
static firmware_header_t app_firmware_header;
static app_update_meta_t app_update_meta;
static uint8_t app_icon_buffer[APP_ICON_SIZE];
static uint32_t app_icon_len = 0;
...
static flash_target_mode_t flash_target_mode = FLASH_TARGET_EXTERNAL;

static bool parse_target_mode(const char* arg) {
	if (arg == NULL) {
		return false;
	}

	if (strcmp(arg, "internal") == 0) {
		flash_target_mode = FLASH_TARGET_INTERNAL;
		return true;
	}

	if (strcmp(arg, "external") == 0) {
		flash_target_mode = FLASH_TARGET_EXTERNAL;
		return true;
	}
...
static bool path_is_directory(const string& path) {
...
static string path_join(const string& dir, const string& name) {
...
static bool resolve_app_folder(const string& folder_path, string* bin_path, string* icon_path) {
	DIR* dir = opendir(folder_path.c_str());
	struct dirent* entry;
	vector<string> bin_candidates;
	vector<string> icon_candidates;
	...
	while ((entry = readdir(dir)) != NULL) {
		...
		if (string_ends_with(name, ".icon.bin")) {
			icon_candidates.push_back(name);
		}
		else if (string_ends_with(name, ".preview.bin")) {
			continue;
		}
		else if (string_ends_with(name, ".bin")) {
			bin_candidates.push_back(name);
		}
	}

	closedir(dir);
	sort(bin_candidates.begin(), bin_candidates.end());
	sort(icon_candidates.begin(), icon_candidates.end());

	if (bin_candidates.size() != 1) {
		APP_DBG(DEBUG_LEVEL_ERROR, "app folder must contain exactly one .bin file: %s\n", folder_path.c_str());
		return false;
	}

	*bin_path = path_join(folder_path, bin_candidates[0]);

	string expected_icon = bin_candidates[0].substr(0, bin_candidates[0].length() - 4) + ".icon.bin";
	string expected_icon_path = path_join(folder_path, expected_icon);
	struct stat icon_info;

	if (stat(expected_icon_path.c_str(), &icon_info) == 0 && S_ISREG(icon_info.st_mode)) {
		*icon_path = expected_icon_path;
	}
	else if (icon_candidates.size() == 1) {
		*icon_path = path_join(folder_path, icon_candidates[0]);
	}
	else {
		icon_path->clear();
	}
...
static string app_name_from_path(const string& path) {
	size_t slash_pos = path.find_last_of("/\\");
	size_t name_pos = (slash_pos == string::npos) ? 0 : slash_pos + 1;
	string name = path.substr(name_pos);
	...
	if (name.length() >= APP_NAME_MAX_LEN) {
		name = name.substr(0, APP_NAME_MAX_LEN - 1);
	}

	return name;
}

static string icon_path_from_firmware_path(const string& path) {
	size_t dot_pos = path.find_last_of('.');

	if (dot_pos == string::npos) {
		return path + ".icon.bin";
	}

	return path.substr(0, dot_pos) + ".icon.bin";
}

static bool load_icon_file(const string& path) {
	struct stat file_info;
	int icon_file = -1;

	memset(app_icon_buffer, 0, sizeof(app_icon_buffer));

	icon_file = open(path.c_str(), O_RDONLY);
	if (icon_file < 0) {
		APP_DBG(DEBUG_LEVEL_WARN, "icon not found, launcher will use text: %s\n", path.c_str());
		return false;
	}

	if (fstat(icon_file, &file_info) < 0 || file_info.st_size != APP_ICON_SIZE) {
		APP_DBG(DEBUG_LEVEL_WARN, "icon must be exactly %u bytes, launcher will use text: %s\n",
				APP_ICON_SIZE, path.c_str());
		close(icon_file);
		return false;
	}

	if (read(icon_file, app_icon_buffer, sizeof(app_icon_buffer)) != (ssize_t)sizeof(app_icon_buffer)) {
		APP_DBG(DEBUG_LEVEL_WARN, "icon read failed, launcher will use text: %s\n", path.c_str());
		memset(app_icon_buffer, 0, sizeof(app_icon_buffer));
		close(icon_file);
		return false;
	}
...
static void transfer_payload_read(uint8_t* data, uint32_t cursor, uint32_t size) {
	uint32_t copied = 0;

	memset(data, 0, size);

	if (cursor < app_firmware_header.bin_len) {
		uint32_t app_read_len = app_firmware_header.bin_len - cursor;

		if (app_read_len > size) {
			app_read_len = size;
		}

		firmware_read(data, cursor, app_read_len, firmware_path.c_str());
		copied += app_read_len;
		cursor += app_read_len;
	}

	if (copied < size) {
		uint32_t icon_cursor = cursor - app_firmware_header.bin_len;
		uint32_t icon_read_len = size - copied;

		if (icon_cursor < APP_ICON_SIZE) {
			if ((icon_cursor + icon_read_len) > APP_ICON_SIZE) {
				icon_read_len = APP_ICON_SIZE - icon_cursor;
			}

			memcpy(&data[copied], &app_icon_buffer[icon_cursor], icon_read_len);
		}
	}
}

static uint16_t transfer_payload_checksum(uint32_t payload_len) {
	uint32_t check_sum = 0;
	uint8_t word_buf[sizeof(uint32_t)];

	for (uint32_t index = 0; index < payload_len; index += sizeof(uint32_t)) {
		uint32_t temp_data = 0;
		uint32_t read_len = (payload_len - index) >= sizeof(uint32_t) ? sizeof(uint32_t) : (payload_len - index);

		transfer_payload_read(word_buf, index, read_len);
		memcpy(&temp_data, word_buf, read_len);
		check_sum += temp_data;
	}

	return (uint16_t)(check_sum & 0xFFFF);
}

static void prepare_external_payload_metadata(void) {
	string app_name = app_name_from_path(firmware_path);
	string icon_path = icon_path_override.empty() ?
			icon_path_from_firmware_path(firmware_path) :
			icon_path_override;

	app_icon_len = load_icon_file(icon_path) ? APP_ICON_SIZE : 0;

	memset(&app_update_meta, 0, sizeof(app_update_meta));
	app_update_meta.magic = APP_UPDATE_META_MAGIC;
	app_update_meta.app_len = app_firmware_header.bin_len;
	app_update_meta.app_checksum = app_firmware_header.checksum;
	app_update_meta.icon_len = app_icon_len;
	strncpy(app_update_meta.name, app_name.c_str(), APP_NAME_MAX_LEN - 1);

	file_firmware_header.bin_len = app_update_meta.app_len + app_update_meta.icon_len;
	file_firmware_header.checksum = transfer_payload_checksum(file_firmware_header.bin_len);

	APP_DBG(DEBUG_LEVEL_INFO, "app.name                 :%s\n", app_update_meta.name);
	APP_DBG(DEBUG_LEVEL_INFO, "app.bin_len              :%u\n", app_update_meta.app_len);
	APP_DBG(DEBUG_LEVEL_INFO, "app.checksum             :0x%04X\n", (uint16_t)app_update_meta.app_checksum);
	APP_DBG(DEBUG_LEVEL_INFO, "icon.path                :%s\n", icon_path.c_str());
	APP_DBG(DEBUG_LEVEL_INFO, "icon.len                 :%u\n", app_update_meta.icon_len);
	APP_DBG(DEBUG_LEVEL_INFO, "payload.bin_len          :%u\n", file_firmware_header.bin_len);
	APP_DBG(DEBUG_LEVEL_INFO, "payload.checksum         :0x%04X\n", file_firmware_header.checksum);
}
...
#define UART_BOOT_CMD_UPDATE_REQ_SIG_INTERVAL		10
...
static bool firmware_vector_matches_target(const char* path, uint32_t target_addr) {
	uint32_t vector[2] = {0, 0};

	if (firmware_read((uint8_t*)vector, 0, sizeof(vector), path) != 0) {
		return false;
	}

	uint32_t stack = vector[0];
	uint32_t reset = vector[1] & ~1u;

	return stack >= 0x20000000u &&
		stack <= 0x20004000u &&
		reset >= target_addr &&
		reset < (target_addr + file_firmware_header.bin_len);
}
...
int main(int argc, char *argv[]) {
	 ...
	 * host_uart_boot device_path file_path start_flash_address [internal|external]
	 * example: host_uart_boot /dev/ttyUSB0 build/file_name.bin 0x08004800 external
	 */
	...
	icon_path_override.clear();

	if (path_is_directory(firmware_path)) {
		string folder_path = firmware_path;

		if (!resolve_app_folder(folder_path, &firmware_path, &icon_path_override)) {
			flash_set_state(FLASH_STATE_FAILED, "invalid app folder");
			return -1;
		}

		APP_DBG(DEBUG_LEVEL_INFO, "app_folder              :%s\n", folder_path.c_str());
		APP_DBG(DEBUG_LEVEL_INFO, "resolved_app_bin        :%s\n", firmware_path.c_str());
		if (!icon_path_override.empty()) {
			APP_DBG(DEBUG_LEVEL_INFO, "resolved_icon           :%s\n", icon_path_override.c_str());
		}
	}
	...
	if (firmware_get_info(&file_firmware_header, firmware_path.c_str()) == 0) {
		memcpy(&app_firmware_header, &file_firmware_header, sizeof(firmware_header_t));
	...
	if (argc >= 5) {
		if (!parse_target_mode(argv[4])) {
			flash_set_state(FLASH_STATE_FAILED, "invalid flash mode");
			APP_DBG(DEBUG_LEVEL_ERROR, "invalid flash mode: %s\n", argv[4]);
			APP_DBG(DEBUG_LEVEL_INFO, "%s\n", help_string.c_str());
			return -1;
		}
	}

	APP_DBG(DEBUG_LEVEL_INFO, "flash_target_mode: %s\n",
			flash_target_mode == FLASH_TARGET_INTERNAL ? "internal" : "external");

	if (flash_target_mode == FLASH_TARGET_EXTERNAL) {
		prepare_external_payload_metadata();
		memcpy(&transfer_fw_status.fw_header, &file_firmware_header, sizeof(firmware_header_t));
	}

	if (flash_target_mode == FLASH_TARGET_INTERNAL &&
			!firmware_vector_matches_target(firmware_path.c_str(), target_des_addr)) {
		uint32_t vector[2] = {0, 0};
		firmware_read((uint8_t*)vector, 0, sizeof(vector), firmware_path.c_str());
		flash_set_state(FLASH_STATE_FAILED, "firmware vector does not match target address");
		APP_DBG(DEBUG_LEVEL_ERROR, "firmware stack: 0x%08X reset: 0x%08X target: 0x%08X\n",
				vector[0], vector[1], target_des_addr);
		return -1;
	}
...
void uart_boot_cmd_handshake_req(void* boot_obj) {
	...
	firmware_boot_cmd.cmd		= SYS_BOOT_CMD_UPDATE_NONE;
	firmware_boot_cmd.container	=
		(flash_target_mode == FLASH_TARGET_INTERNAL)
			? SYS_BOOT_CONTAINER_DIRECTLY
			: SYS_BOOT_CONTAINER_EXTERNAL_FLASH;
...
void uart_boot_cmd_update_req(void*) {
	...
	if (flash_target_mode == FLASH_TARGET_EXTERNAL) {
		uart_boot_data_cmd.len += sizeof(app_update_meta_t);
		memcpy(&uart_boot_data_cmd.data[sizeof(firmware_header_t)], &app_update_meta, sizeof(app_update_meta_t));
	}
...
void uart_boot_cmd_transfer_fw_req(void*) {
	...
	transfer_payload_read(fw_frame_buffer, firmware_transfer_index, fw_frame_len);
	...
	if (firmware_transfer_index <= file_firmware_header.bin_len && checksum_flag == 0) {
		...
		uart_boot_data_cmd.boot_cmd.subcmd	= 0;
		uart_boot_data_cmd.len				= fw_frame_len;
...
void uart_boot_cmd_checksum_fw_res(void* boot_obj) {
	...
	if (boot_cmd->cmd == UART_BOOT_CMD_CHECKSUM_FW_RES) {
		...
		else if (boot_cmd->subcmd == UART_BOOT_SUB_CMD_3) {
			flash_set_state(FLASH_STATE_FAILED, "target app vector invalid");
			APP_DBG(DEBUG_LEVEL_ERROR, "target app vector does not match destination address\n");
			exit(0);
		}
		else {
