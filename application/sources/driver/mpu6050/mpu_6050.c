...
// Setup MPU6050. Values are 8-bit STM32 SPL addresses: AD0 low = 0xD0, AD0 high = 0xD2.
#define MPU6050_ADDR_LOW        0xD0
#define MPU6050_ADDR_HIGH       0xD2
#define MPU6050_WHO_AM_I_VALUE  0x68
#define MPU6050_INIT_RETRIES    20
const uint16_t i2c_timeout = 100;
const double Accel_Z_corrector = 14418.0;
uint32_t timer;
static uint8_t mpu6050_addr = MPU6050_ADDR_LOW;
static uint8_t mpu6050_who_am_i = 0;
...
{
	volatile uint32_t delay = 72000;
	while(delay--);
}

static bool MPU6050_ProbeAddress(uint8_t address)
{
	mpu6050_who_am_i = i2c_hw_read(address, WHO_AM_I_REG);
...
{
	mpu6050_ready = false;
	for(uint8_t retry = 0; retry < MPU6050_INIT_RETRIES; retry++)
	{
		if(MPU6050_ProbeAddress(MPU6050_ADDR_LOW) || MPU6050_ProbeAddress(MPU6050_ADDR_HIGH))
		{
			i2c_hw_write(mpu6050_addr, PWR_MGMT_1_REG, 0x80);
			MPU6050_Delay();
			i2c_hw_write(mpu6050_addr, PWR_MGMT_1_REG, 0x01);
			MPU6050_Delay();
...
{		
    ...
    // Kalman angle solve
    double dt = (double)(akos_thread_get_tick() - timer) / 1000;
    timer = akos_thread_get_tick();
    double roll;
